document.addEventListener('DOMContentLoaded', () => {
  ...
  const worksData = {
    'typing-rouge': {
      title: '卒業研究（ゲーム名未定）',
      altText: '卒業研究',
      images: ['img/samples/typing_rouge1.png', 'img/samples/typing_rouge2.png', 'img/samples/typing_rouge3.png'],
      videos: [],
      description: `
      <p>卒業研究のために、タイピングゲームとローグライトを掛け合わせた戦闘ゲームの制作しています（卒業研究では UI の認識しやすさに関する研究を行います）。</p>
      <p>6月下旬現在ではエリア全般の処理があらかた完成し、戦闘部分のゲームフローが完成したので、最低限ゲームとして遊べるようになりました。
      ここからは、細かな仕様を実装しつつ、UIなどのグラフィック面に手をつけていこうと考えています。</p><br />
      <p>また本作はより本格的なゲームにするために、大まかな世界観を決めて制作しています。
      本作は、【魔力で動作する機械】で栄えた、人間と魔族が共存している国が舞台で、戦闘に使う武器・装備・魔導具も機械製、魔力駆動の機械なので魔法を纏うことも可能です。
      国が所有する軍隊に所属する軍人、街で整備士業を営む少年、といったキャラクターを実装予定です（3枚目の画像は軍人のデザインラフ）。</p><br />
      <p>プログラムに加えてイラストも自作するため、個人開発としては規模の大きなゲームになります。
      加えて制作猶予が1年未満と非常に短いため、開発補助として生成AIを活用しています（設計方針の決定や要件整理、コードチェックやバグの修正などは自身で行っています）。</p>
      <p>かなり大変な作業にはなりますが、頑張って完成させていこうと思ってます！</p>
      `,
      period: '開発中（2026年4月〜）',
      dev: 'Unity, ChatGPT, GitHub Copilot', 
      links: [
        { url: 'https://x.com/yukineko_K1', text: '現状の開発状況を投稿する作者X（旧Twitter）はこちら' }
      ]
    },
    'urasoroe': {
      title: '<i class="fa-regular fa-heart"></i> ウラソロエ',
      altText: 'ウラソロエ',
      ...
      description: `
        ...
        <p>表面に柄が描かれているカードおり、2枚裏返して「柄」と「色」が一致すれば、カードを取得することが出来る神経衰弱のようなゲームです。</p><br />
        <p>今回は神経衰弱部分のプログラムと画像素材の作成、その他 UI とアニメーションを担当しました。
        カードやボタン、カウントダウン時のデザインなども自分で考えて描きました。</p>
        <p>友人が作成した実績要素は今後制作するゲームでかなり応用が利きそうなので、極力実装していくようにしたいと考えています！</p>
      `,
      period: '6日間（2026/03/17〜2026/03/22）',
      dev: 'Unity',
      links: [
    ...
    'color-run': {
      ...
      description:`
        <p>プレイヤーの色と障害物の色を合わせることで、障害物にぶつからなくなるランゲームです。
        （現状、ステージと UI の一部が未完成となっています。卒業研究でのゲーム制作の準備に移行したため開発停止中。）</p>
        ...
        <p>自分の中にある理想を少しずつ形にする作業が非常に面白かったです！</p>
      `,
      period: '約1ヶ月（2025年9月上旬〜9月下旬）',
      dev: 'Unity',
      links: []
    },
    'minesweeper': {
      ...
      description: `
        ...
        <p>『14 Minesweeper Variants』という特殊ルール付きのマインスイーパーが好きなので、ゲーム内に登場する2つのルールを再現してみました。
        本家様のように特殊ルールを巧みに使った手筋などはありませんが、ルールを再現できて良かったです。</p>
        <p>工夫点として、理不尽なゲームオーバーを避けるために、最初は確定でマスが開けられるように設定しています。
        また、数字や旗に色を付けることで、ターミナル上での見やすさにこだわりました。</p><br />
        <p>【2026/06/04追記】Mac 上で動作するようにアップデートを行いました。</p>
      `,
      period: '3日（2025年8月下旬）',
      dev: 'C++',
      links: [
    ...
    'ice-hopper': {
      ...
      description: `
        ...
        <p>今回も友人と参加しました！ゆきねこ｡は主にプレイヤー関連の処理、床の処理、チュートリアル以外のステージ制作を担当しました。</p>
      `,
      period: '1週間（2025/04/13〜2025/4/20）',
      dev: 'Unity',
      links: [
    ...
    'akari-puzzle': {
      ...
      description: `
        ...
        <p>（移動の処理を条件文で強引に実装したせいでコードが悲惨なことになっているので、もしかしたら改善するかもしれません）</p>
      `,
    ...
    'survive': {
      ...
      description: `
        ...
        <p>（実は2025年度の高専祭で展示した際に不具合が発覚したため、近いうちに修正を行う予定です）</p>
        <p>（↑自身の持っていた制作データが大きく破損しており、プロジェクト内のデータがほとんど消滅した状態となっていました。
        さらに他の制作メンバーに確認したところ、誰も最終盤の制作データを持っていないことが判明。
        当時は Git を使用していなかったためデータが復旧できず、修正は断念しました。）</p>
      `,
      period: '約2ヶ月',
      dev: 'Unity',
      links: [
    ...
    'collect-money': {
      ...
      `,
      period: '1週間（2024/12/22〜2024/12/29）',
      dev: 'Unity',
      links: [
